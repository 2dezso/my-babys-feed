...
**"Start feed"** and **"Log a feed"** sit side by side as one merged split button (`.feed-action-row`, two `.feed-action-btn` halves sharing one rounded/shadowed container so it reads as a single control). Start feed is the one-tap way to log a pending feed (`startFeed()`) — no modal, just the current time, for logging fast mid-feed and filling in the amount later. Log a feed opens the modal for a fully-specified feed (own time/amount/interval).

While a feed is pending, the right half ("Log a feed") disappears entirely and the left button morphs in place — same element, just relabeled **"✅ Complete feed"** (icon, text, and color swap from orange to green via the `.feed-action-complete` class) and now spanning the full width, since flexbox reclaims the space the hidden half left behind. Its click handler branches on whether a feed is pending: pending → opens the log modal in "Add amount" mode (same as tapping the hero card); not pending → starts a new one. This avoids the previous confusing state where "Start feed" and "Log a feed" sat there unchanged during a pending feed — tapping "Start feed" again would silently create a second orphaned pending feed, and "Log a feed" would open an unrelated new entry instead of finishing the one in progress. There's deliberately no separate "Complete feed" element — reusing the Start feed button avoids a jarring swap-in of a brand-new button.
